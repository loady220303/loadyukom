...
function Home(props) {
    ...
    let kota2;
    if(kota.results){
        kota2 = kota.results.map(function(val){
            // console.log({value: val.city_id, label: val.city_name })
            return {value: val.city_id, label: val.city_name }
        })
    };
    ...
    const selectOriginCity = (e) => {
        ...
        setNamaKotaAsal(e.label);
        setKotaAsal(e.value);
        setVisibleOrigin(false);
    };

    const selectDestination = (e) => {
        setNamaKotaTujuan(e.label);
        setKotaTujuan(e.value);
    ...
    const cekOngkir = (e) => {
        setLoading(true);
        axios
            .post("https://cekresi-ongkir-indonesia-4yo61j1y7.vercel.app/cek_harga", {
                kota_asal: kotaAsal,
                type_asal: "city",
    ...
    const cekResi = (e) => {
        ...
        setError(false);
        axios
            .post("https://cekresi-ongkir-indonesia.revell29.vercel.app/check_resi", {
                resi: data.resi,
                kurir: data.kurir,
            })
            .then((res) => {
    ...
    useEffect(() => {
        axios.get("https://cekresi-ongkir-indonesia.revell29.vercel.app/kota").then((res) => setKota(res.data.data));
    }, []);
    ...
    return (
        <>
            ...
            {visibleOrigin && (
                <div className="h-full w-full">
                    ...
                    <Modal className="bg-white bottom-0 p-5 left-0 right-0 rounded-t-lg absolute z-50">
                        <h1 className="text-3xl text-grey-500 font-semibold">Pilih Kota Asal</h1>
                        <Select className="w-full p-5 mt-3 bg-gray-300 rounded-lg focus:outline-none" options={kota2} onChange={(e) => selectOriginCity(e)}>
                        </Select>
                    </Modal>
                </div>
            )}
            {visibleDesti && (
                <div className="h-full w-full">
                    ...
                    <Modal className="bg-white bottom-0 p-5 left-0 right-0 rounded-t-lg absolute z-50">
                        ...
                        <Select options={kota2} className="w-full p-5 mt-3 bg-gray-300 rounded-lg focus:outline-none" onChange={(e) => selectDestination(e)}>
                        </Select>
                    </Modal>
                </div>
            )}
            <Section>
                <Section2>
                    <header className="flex items-center justify-between p-5">
                        <h1 className="text-2xl uppercase text-blue-500">CEK KURIR</h1>
            ...
            <Footer>
                <p className="ml-4">
                    Build With<span className="mr-2">❤</span>LoadyFiqriA
                </p>
            </Footer>
        </>
    );
